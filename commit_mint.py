from datetime import datetime, timedelta, date
import random
import os
import shutil
import subprocess

source = 'git@github.com'

def roundTime(dt=None, roundTo=60):
   if dt == None:
      dt = datetime.datetime.now()
   seconds = (dt.replace(tzinfo=None) - dt.min).seconds
   rounding = (seconds+roundTo/2) // roundTo * roundTo
   return dt + timedelta(0, rounding-seconds, -dt.microsecond)

#takes the three arguments username, repo name, and number of days back
def fake_commit(user,repo,days):
    #this is the start of the bash script
    beginning = """#!/usr/bin/env bash
                    REPO={0}
                    git init $REPO
                    cd $REPO
                    touch README.md
                    git add README.md
                    touch dev_fake_commit
                    git add dev_fake_commit
                """.format(repo)
    #ending writes the end of the bash script
    # puts source:user ie. git@github.com:username
    ending = """
            git remote add origin {0}:{1}/$REPO.git
            git pull origin master
            git push -u origin master
            """.format(source,user)
    #lines is the content of the bash script as an array
    lines = []
    #beginning is now in the script 
    lines.append(beginning)
    
    # commitdate = datetime.today()
    today = date.today()
    yyyy = int(input("input end year... "))
    mm = int(input("end month... "))
    dd = int(input("end day... "))
    a = date(yyyy,mm,dd)
    b = date(today.year, today.month,today.day)
    days_since_end = (b-a).days

    for i in range(days_since_end,days):
        rnd = random.randint(0, 12)
        commitdate = roundTime(
            datetime.today() - timedelta(days=i), roundTo=60*60)
        if commitdate.isoweekday() != 7 and commitdate.isoweekday() != 6:
            for j in range(rnd):
                template = '''GIT_AUTHOR_DATE={0} GIT_COMMITTER_DATE={1} git commit --allow-empty -m "dev" > /dev/null\n'''.format(commitdate.isoformat(), commitdate.isoformat())
                lines.append(template)

    lines.append(ending)
    with open('commits.sh', 'w') as f:
        for line in lines:
            f.write(line)
    os.chmod('commits.sh', 0o755)

def main():
    username = input("Enter Username: ")
    repo = input('Enter repository name to fake commits: ')
    days = int(input('Enter number of days to go back in time: '))
    
    fake_commit(username, repo, days)
    subprocess.call(['./commits.sh'])

if __name__ == '__main__':
    main()
